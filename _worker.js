...
const DNS_TIMEOUT_MS = 5000, DNS_TTL_MS = 120000, DNS_NEGATIVE_TTL_MS = 5000;
const dnsCache = new Map();
const withTimeout = (promise, ms, message = 'operation timeout') => Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error(message)), ms))
]);
const cachedDnsResolve = async (hostname, recordType) => {
    const key = `${recordType}:${hostname}`;
    const now = Date.now(), cached = dnsCache.get(key);
    if (cached && cached.expires > now) return cached.value;
    try {
        const value = await withTimeout(Promise.any(dohNatEndpoints.map(endpoint =>
            fetch(`${endpoint}?name=${encodeURIComponent(hostname)}&type=${recordType}`, dohJsonOptions).then(response => {
                if (!response.ok) throw new Error();
                return response.json();
            })
        )), DNS_TIMEOUT_MS, 'DNS timeout');
        const answer = value.Answer || value.answer;
        const result = answer?.length ? answer : null;
        dnsCache.set(key, {value: result, expires: now + (result ? DNS_TTL_MS : DNS_NEGATIVE_TTL_MS)});
        if (dnsCache.size > 256) dnsCache.delete(dnsCache.keys().next().value);
        return result;
    } catch {
        dnsCache.set(key, {value: null, expires: now + DNS_NEGATIVE_TTL_MS});
        return null;
    }
};
...
let currentColo = null, currentColoExpires = 0;
const getCurrentColo = async () => {
    if (Date.now() < currentColoExpires) return currentColo || '';
    try {
        ...
        currentColoExpires = Date.now() + (currentColo ? 600000 : 30000);
        return currentColo;
    } catch {
        currentColo = null;
        currentColoExpires = Date.now() + 30000;
        return '';
    }
};
...
const dohDnsHandler = async (payload) => {
    ...
    const resp = await withTimeout(Promise.any(dohEndpoints.map(endpoint =>
        fetch(endpoint, {method: 'POST', headers: dohHeaders, body: dnsQueryData}).then(response => {
            if (!response.ok) throw new Error();
            return response;
        })
    )), DNS_TIMEOUT_MS, 'DoH timeout');
...
const getSub = async (request, url, uuid) => {
    if (uuid && url.searchParams.get('uuid') !== uuid) return errorResponse();
